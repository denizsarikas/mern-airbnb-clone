import axios from "axios"
import { useEffect, useState } from "react"
import { Link } from "react-router-dom";
import Image from "../components/Image";



export default function IndexPage() {

    const [places, setPlaces] = useState([]);

    useEffect(() => {
        axios.get('/places').then(response => {
            setPlaces(response.data)
            // setPlaces([...response.data, ...response.data])
            // console.log(places)
        });
    }, []);


    return (
        <div className='gap-x-6 gap-y-8 mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>

            {places.length > 0 && places.map(place => (
                <Link to={'/place/' + place._id}>
                    <div className="bg-gray-500 mb-2 rounded-2xl flex">
                        {place.photos?.[0] && (
                            // <img className="rounded-2xl object-cover aspect-square" src={'http://localhost:4000/uploads/' + place.photos?.[0]} alt="photo" />
                            <Image className="rounded-2xl object-cover aspect-square" src={place.photos?.[0]} alt="photo" />
                        )}
                    </div>
                    <h2 className="font-bold leading-1">{place.address}</h2>
                    <h3 className="text-sm truncate leading-1 text-gray-500">{place.title}</h3>
                    <div className="mt-2">
                        <span className="font-bold mr-1"> ${place.price} </span>per night
                    </div>
                </Link>
            ))}
        </div>
    )
}
